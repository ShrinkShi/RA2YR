...
| U-03 | flags 2的真实字节流行为 | 位模型可表达`UsesRle=true`, `HasTransparency=false`；XCC按RLE-Zero；OpenRA按length-prefixed raw scanlines | 找到真实样本或官方/编辑器写入证据 | source-conflicting / underconfirmed；不作为首版正常写入目标；黄金审计前不选择默认解码策略 |
...
| U-11 | `00 00`的严格语义 | 已确认消费2个输入字节、输出0像素；no-op/padding/非法三种解释未收敛 | 工具行为、本地黄金样本、必要时原版实验 | 命令预算；输入推进；行输入精确消费；最终输出等于width；不预先决定严格接受性 |
...
| U-16 | SHP(TS)是否存在任何reference/delta扩展 | 强来源均无字段 | 未来单独研究；先排除家族误判，再要求明确样本和许可证来源 | 首版不推荐`ShpFrameDependency`、不实现resolver、不增加依赖预算；parser只测试不会发明dependency |
...
| U-21 | C&C Remastered旧家族证据的精确commit | 当前只固定仓库，未固定commit | 固定具体commit和相关路径后再提升来源状态 | supplementary only，不进入主要证据权重 |
