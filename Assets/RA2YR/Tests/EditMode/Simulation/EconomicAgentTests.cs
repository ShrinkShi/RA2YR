...
namespace RA2YR.Tests.EditMode.Simulation
{
    public sealed class EconomicAgentTests
    {
        ...
        [Test] public void NegativePowerProducesExplicitBuildPowerCandidate() { var o = Observation(); var r = EconomicAgentEvaluator.Evaluate(new EconomicAgentObservation(o.Tick, o.Credits, o.Resources, o.Cargo, o.ProductionDefinition, o.Production, new StructurePowerProjection(new[] { new StructureDefinitionRaw(0, "Plant", 1, 1, 100, 0, 10, "Player0") }), o.Interaction), EconomicAgentPolicy.ConservativeDeterministic, AgentLimits()); Assert.That(r.Proposals, Has.Some.Matches<EconomicAgentActionProposal>(x => x.Kind == EconomicAgentActionKind.BuildPowerCandidate)); }
        [Test] public void MissingFactoryProducesExplicitBuildFactoryCandidate() { var o = Observation(); var r = EconomicAgentEvaluator.Evaluate(o, EconomicAgentPolicy.ConservativeDeterministic, EconomicAgentStrategyProfile.Balanced, AgentLimits()); Assert.That(r.Proposals, Has.Some.Matches<EconomicAgentActionProposal>(x => x.Kind == EconomicAgentActionKind.BuildFactoryCandidate)); Assert.That(o.Needs, Does.Contain(EconomicAgentNeed.NeedFactory)); }
        [Test] public void NeuralUnavailableUsesExplicitRuleBasedFallbackContract() { var r = EconomicAgentEvaluator.EvaluateWithBackend(Observation(), EconomicAgentPolicy.ConservativeDeterministic, EconomicAgentStrategyProfile.Balanced, AgentLimits(), null); Assert.That(r.BackendStatus, Is.EqualTo(EconomicAgentBackendStatus.NeuralUnavailable)); Assert.That(r.Proposals, Is.Not.Empty); }
        [Test] public void StrategyProfileChangesPriorityWithoutChangingCandidateIdentity() { var rush = EconomicAgentEvaluator.Evaluate(Observation(), EconomicAgentPolicy.ConservativeDeterministic, EconomicAgentStrategyProfile.Rush, AgentLimits()); var macro = EconomicAgentEvaluator.Evaluate(Observation(), EconomicAgentPolicy.ConservativeDeterministic, EconomicAgentStrategyProfile.Macro, AgentLimits()); Assert.That(rush.Proposals.Select(x => x.Kind), Is.EquivalentTo(macro.Proposals.Select(x => x.Kind))); Assert.That(rush.Proposals.First(x => x.Kind == EconomicAgentActionKind.HarvestCandidate).Priority, Is.Not.EqualTo(macro.Proposals.First(x => x.Kind == EconomicAgentActionKind.HarvestCandidate).Priority)); }
