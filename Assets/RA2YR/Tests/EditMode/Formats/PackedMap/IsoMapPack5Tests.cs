...
namespace RA2YR.Tests.EditMode.Formats.PackedMap
{
    public sealed class IsoMapPack5Tests
    {
        ...
        [Test]
        public void NoTmpModelsWereAdded() { Assert.That(typeof(IsoMapPack5RecordRaw).Assembly.GetTypes().Where(t => t.Namespace != null && t.Namespace.StartsWith("RA2YR.Core.Formats.PackedMap", StringComparison.Ordinal)).Select(t => t.Name), Has.None.Matches<string>(name => name.Equals("TMP", StringComparison.OrdinalIgnoreCase))); }
