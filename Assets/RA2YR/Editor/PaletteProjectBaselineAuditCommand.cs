...
{
    public static class PaletteProjectBaselineAuditCommand
    {
        private const int MaximumSummaryUtf8Bytes = 1024 * 1024;
        private const int ExpectedPaletteCount = 3;
        ...
        public static void Run()
        {
            try
            {
                ...
                Debug.Log(
                    "WP-02D PAL ProjectBaseline audit completed: status=" + delivery.Status +
                    ", palettes=" + delivery.PaletteCount +
            ...
            ExternalContentConfiguration configuration)
        {
                ...
                enabled[0].Kind != ContentSourceKind.Patched)
            {
                throw new InvalidOperationException(
                    "The controlled PAL audit requires exactly one enabled patched " +
            ...
            string summaryOutput)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized PAL summary output must be a JSON file below TestResults.");
            }
            ...
            {
                throw new IOException("The sanitized PAL summary output path already exists.");
            }
        }
        ...
        private static void ValidateRegularInputFile(string path)
        {
            ...
            {
                throw new IOException("A controlled PAL audit input is not a regular file.");
            }
        }
        ...
        private static void RejectExistingReparsePoint(string path)
        {
            ...
            {
                throw new IOException("A controlled PAL audit path traverses a reparse point.");
            }
        }
            ...
            string repositoryRoot)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The PAL audit did not produce a sanitized summary.");
            }

            if (delivery.Status != PaletteProjectBaselineAuditStatus.Complete ||
                delivery.PaletteCount != ExpectedPaletteCount ||
                delivery.ExternalManifestLength <= 0 ||
                !IsLowerSha256(delivery.ExternalManifestSha256))
            {
                throw new InvalidOperationException(
                    "The PAL audit returned an incomplete or inconsistent delivery.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized PAL audit summary exceeds its delivery budget.");
            }
            ...
            if (!delivery.SanitizedSummaryJson.Contains(
                ...
                !delivery.SanitizedSummaryJson.Contains("\"palettes\":["))
            {
                throw new InvalidOperationException(
                    "The sanitized PAL summary does not match the approved delivery identity.");
            }
        }
        ...
        private static void RejectSensitivePath(string summary, string path)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized PAL summary contains a protected host path.");
            }
        }
