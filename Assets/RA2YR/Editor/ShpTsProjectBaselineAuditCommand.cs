...
{
    public static class ShpTsProjectBaselineAuditCommand
    {
        private const int MaximumSummaryUtf8Bytes = 2 * 1024 * 1024;
        private const int ExpectedSampleCount = 6;
        private const int ExpectedUnresolvedFrameCount = 0;
        private const int ExpectedFailedFrameCount = 257;
        ...
        public static void Run()
        {
            try
            {
                ...
                Debug.Log(
                    "M2-SHP1 ProjectBaseline audit completed: status=" + delivery.Status +
                    ", samples=" + delivery.SampleCount +
                    ", unresolvedFrames=" + delivery.UnresolvedFrameCount +
                    ", failedFrames=" + delivery.FailedFrameCount +
            ...
            ExternalContentConfiguration configuration)
        {
                ...
                enabled[0].Kind != ContentSourceKind.Patched)
            {
                throw new InvalidOperationException(
                    "The controlled SHP audit requires exactly one enabled patched " +
            ...
            string summaryOutput)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP summary output must be a JSON file below TestResults.");
            }
            ...
            {
                throw new IOException("The sanitized SHP summary output path already exists.");
            }
        }
        ...
        private static void ValidateRegularInputFile(string path)
        {
            ...
            {
                throw new IOException("A controlled SHP audit input is not a regular file.");
            }
        }
        ...
        private static void RejectExistingReparsePoint(string path)
        {
            ...
            {
                throw new IOException("A controlled SHP audit path traverses a reparse point.");
            }
        }
            ...
            string repositoryRoot)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The SHP audit did not produce a sanitized summary.");
            }

            if (delivery.Status !=
                    ShpTsProjectBaselineAuditStatus.CompleteWithDecodeFailures ||
                delivery.SampleCount != ExpectedSampleCount ||
                delivery.UnresolvedFrameCount != ExpectedUnresolvedFrameCount ||
                delivery.FailedFrameCount != ExpectedFailedFrameCount ||
                delivery.ExternalManifestLength <= 0 ||
                !IsLowerSha256(delivery.ExternalManifestSha256))
            {
                throw new InvalidOperationException(
                    "The SHP audit returned an incomplete or inconsistent delivery.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP audit summary exceeds its delivery budget.");
            }
            ...
            if (!delivery.SanitizedSummaryJson.Contains(
                ...
                !delivery.SanitizedSummaryJson.Contains(
                    "\"baselineLogicalName\":\"YR1001_ProjectBaseline\"") ||
                !delivery.SanitizedSummaryJson.Contains(
                    "\"auditStatus\":\"CompleteWithDecodeFailures\"") ||
                !delivery.SanitizedSummaryJson.Contains("\"samples\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"frames\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"indices\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"logicalName\":"))
            {
                throw new InvalidOperationException(
                    "The sanitized SHP summary does not match the approved delivery identity.");
            }
        }
        ...
        private static void RejectSensitivePath(string summary, string path)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP summary contains a protected host path.");
            }
        }
