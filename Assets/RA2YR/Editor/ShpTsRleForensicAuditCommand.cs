...
{
    public static class ShpTsRleForensicAuditCommand
    {
        private const int MaximumSummaryUtf8Bytes = 2 * 1024 * 1024;
        private const int ExpectedStageAFrameCount = 257;
        ...
        public static void Run()
        {
            try
            {
                ...
                ShpTsRleForensicAuditDelivery delivery =
                ...
                Debug.Log(
                    "M2-SHP1F forensic audit completed: stageAFrames=" +
                    delivery.StageAFrameCount +
                    ", stageBExecuted=" + delivery.StageBExecuted +
                    ", stageBRows=" + delivery.StageBRowCount +
                    ", decision=" + delivery.Decision +
                    ", productionRepairRecommended=" +
                    delivery.ProductionRepairRecommended +
            ...
            ExternalContentConfiguration configuration)
        {
                ...
                enabled[0].Kind != ContentSourceKind.Patched)
            {
                throw new InvalidOperationException(
                    "The controlled SHP forensic audit requires exactly one enabled " +
                    "patched YR1001_ProjectBaseline source.");
            }
        }
            ...
            string summaryOutput)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP forensic summary must be a JSON file below TestResults.");
            }
            ...
            if (File.Exists(output) || Directory.Exists(output))
            {
                throw new IOException(
                    "The sanitized SHP forensic summary output path already exists.");
            }
        }
        ...
        private static void ValidateRegularInputFile(string path)
        {
            ...
            {
                throw new IOException(
                    "A controlled SHP forensic audit input is not a regular file.");
            }
        }
        ...
        private static void RejectExistingReparsePoint(string path)
        {
            ...
            {
                throw new IOException(
                    "A controlled SHP forensic audit path traverses a reparse point.");
            }
        }
            ...
            string repositoryRoot)
        {
            if (delivery == null || string.IsNullOrWhiteSpace(delivery.SanitizedSummaryJson) ||
                delivery.StageAFrameCount != ExpectedStageAFrameCount ||
                delivery.StageBRowCount < 0 ||
                delivery.ExternalManifestLength <= 0 ||
                !IsLowerSha256(delivery.InputCatalogSha256) ||
                !IsLowerSha256(delivery.CanonicalModelSha256) ||
                !IsLowerSha256(delivery.ExternalManifestSha256))
            {
                throw new InvalidOperationException(
                    "The SHP forensic audit returned an incomplete or inconsistent delivery.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP forensic summary exceeds its delivery budget.");
            }
            ...
            if (!delivery.SanitizedSummaryJson.Contains(
                ...
                !delivery.SanitizedSummaryJson.Contains("\"stageA\":{") ||
                !delivery.SanitizedSummaryJson.Contains("\"stageB\":{") ||
                !delivery.SanitizedSummaryJson.Contains("\"hypotheses\":{") ||
                !delivery.SanitizedSummaryJson.Contains("\"decision\":") ||
                delivery.SanitizedSummaryJson.Contains("\"records\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"frameIndex\":") ||
                delivery.SanitizedSummaryJson.Contains("\"sampleId\":"))
            {
                throw new InvalidOperationException(
                    "The sanitized SHP forensic summary does not match the approved identity.");
            }
        }
        ...
        private static void RejectSensitivePath(string summary, string path)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized SHP forensic summary contains a protected host path.");
            }
        }
