...
{
    public static class IsoMapPack5ProjectBaselineAuditCommand
    {
        ...
        public static void Run()
        {
            try
            {
                ...
                if (enabled.Length != 1 ||
                    !string.Equals(enabled[0].Id, IsoMapPack5ProjectBaselineAuditService.BaselineLogicalName, StringComparison.Ordinal) ||
                    enabled[0].Kind != ContentSourceKind.Patched)
                    throw new InvalidOperationException("The controlled IsoMapPack5 audit requires exactly one enabled patched ProjectBaseline source.");

                IsoMapPack5ProjectBaselineAuditDelivery delivery =
                    IsoMapPack5ProjectBaselineAuditService.Run(configuration);
                ...
                Debug.Log("M3-C4 IsoMapPack5 ProjectBaseline audit completed: status=" + delivery.Status +
                    ", candidates=" + delivery.CandidateSectionCount +
                    ", successes=" + delivery.SuccessfulSectionCount +
                    ", failures=" + delivery.FailedSectionCount +
        ...
        private static void ValidateSummaryOutput(string repositoryRoot, string output)
        {
            ...
            if (!normalizedOutput.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) ||
                ...
                throw new InvalidOperationException("The sanitized IsoMapPack5 summary must be a JSON file below TestResults.");
            ...
            string repositoryRoot)
        {
            if (delivery == null || string.IsNullOrWhiteSpace(delivery.SanitizedSummaryJson))
                throw new InvalidOperationException("The IsoMapPack5 audit did not produce a sanitized summary.");
            byte[] utf8 = new UTF8Encoding(false, true).GetBytes(delivery.SanitizedSummaryJson);
            if (utf8.Length > MaximumSummaryUtf8Bytes)
                throw new InvalidOperationException("The sanitized IsoMapPack5 summary exceeds its delivery budget.");
            ...
            if (!delivery.SanitizedSummaryJson.Contains("\"manifestType\":\"RA2YR.IsoMapPack5ProjectBaselineAuditSanitized\"") ||
                delivery.SanitizedSummaryJson.Contains("\"records\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"coordinates\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"payload\":"))
                throw new InvalidOperationException("The sanitized IsoMapPack5 summary contains forbidden detail.");
        }
