...
{
    public static class CsfProjectBaselineAuditCommand
    {
        ...
        public static void Run()
        {
            try
            {
                ...
                Debug.Log(
                    "WP-02E CSF ProjectBaseline audit completed: status=" + delivery.Status +
            ...
            ExternalContentConfiguration configuration)
        {
                ...
                enabled[0].Kind != ContentSourceKind.Patched)
            {
                throw new InvalidOperationException(
                    "The controlled CSF audit requires exactly one enabled patched " +
            ...
            string summaryOutput)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized CSF summary output must be a JSON file below TestResults.");
            }
            ...
            {
                throw new IOException("The sanitized CSF summary output path already exists.");
            }
        }
        ...
        private static void ValidateRegularInputFile(string path)
        {
            ...
            {
                throw new IOException("A controlled CSF audit input is not a regular file.");
            }
        }
        ...
        private static void RejectExistingReparsePoint(string path)
        {
            ...
            {
                throw new IOException("A controlled CSF audit path traverses a reparse point.");
            }
        }
            ...
            string repositoryRoot)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The CSF audit did not produce a sanitized summary.");
            }

            if (delivery.Status != CsfProjectBaselineAuditStatus.Complete ||
                delivery.DocumentCount != 1 ||
                delivery.ExternalManifestLength <= 0 ||
                !IsLowerSha256(delivery.ExternalManifestSha256))
            {
                throw new InvalidOperationException(
                    "The CSF audit returned an incomplete or inconsistent delivery.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized CSF audit summary exceeds its delivery budget.");
            }
            ...
            if (!delivery.SanitizedSummaryJson.Contains(
                ...
                !delivery.SanitizedSummaryJson.Contains("\"csf\":{") ||
                delivery.SanitizedSummaryJson.Contains("\"records\":[") ||
                delivery.SanitizedSummaryJson.Contains("\"mainText\":"))
            {
                throw new InvalidOperationException(
                    "The sanitized CSF summary does not match the approved delivery identity.");
            }
        }
        ...
        private static void RejectSensitivePath(string summary, string path)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The sanitized CSF summary contains a protected host path.");
            }
        }
