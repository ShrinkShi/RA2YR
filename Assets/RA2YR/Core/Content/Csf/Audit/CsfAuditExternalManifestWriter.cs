...
{
    internal static class CsfAuditExternalManifestWriter
    {
        public static CsfAuditExternalManifestReference Write(
            ...
            byte[] manifestBytes)
        {
            ...
            try
            {
                ...
                string relativeDirectory =
                    "wp02e/csf-audits/" + sourceId + "/" + directoryFingerprint;
                ...
                CreateVerifiedDirectory(Path.Combine(configuration.CachePath, "wp02e"));
                ...
                if (!RepositoryPathPolicy.IsInsideOrEqual(
                    ...
                    string.Equals(
                        ...
                        PathComparison))
                {
                    throw Failure("The external CSF manifest escaped its cache boundary.");
                }

                if (File.Exists(targetPath))
                {
                    VerifyExistingFile(targetPath, manifestBytes, manifestSha256);
                    return new CsfAuditExternalManifestReference(
                        relativePath,
            ...
            catch (CsfProjectBaselineAuditException)
            {
                throw;
            }
            catch (Exception exception) when (IsExpectedFileException(exception))
            {
                throw Failure("The repository-external CSF manifest write failed closed.");
            }
        }
            ...
            ExternalContentConfiguration configuration)
        {
            ...
            foreach (string protectedPath in protectedPaths)
            {
                ...
                {
                    throw Failure(
                        "The external CSF cache boundary could not be verified safely.");
                }
            }
            ...
            if (RepositoryPathPolicy.TryFindUnsupportedAlias(
                ...
                RepositoryPathPolicy.ContainsExistingReparsePoint(
                    ...
                    out reparsePoint))
            {
                throw Failure("The external CSF cache uses an unsupported path identity.");
            }
        }
        ...
        private static void CreateVerifiedDirectory(string path)
        {
            if (File.Exists(path))
            {
                throw Failure("An external CSF cache directory is occupied by a file.");
            }

            string reparsePoint;
            if (RepositoryPathPolicy.ContainsExistingReparsePoint(path, out reparsePoint))
            {
                throw Failure("An external CSF cache directory traverses a reparse point.");
            }
        ...
        private static void EnsureRegularDirectory(string path)
        {
            ...
            {
                throw Failure("An external CSF cache path is not a regular directory.");
            }
        }
            ...
            string expectedSha256)
        {
            ...
            if ((attributes & FileAttributes.Directory) != 0 ||
                (attributes & FileAttributes.ReparsePoint) != 0)
            {
                throw Failure("An external CSF manifest is not a regular file.");
            }
            ...
            {
                throw Failure(
                    "An existing content-addressed CSF manifest does not match its expected bytes.");
            }
        }
        ...
        private static CsfProjectBaselineAuditException Failure(string message)
        {
            return new CsfProjectBaselineAuditException(
                CsfProjectBaselineAuditFailureCode.ExternalManifestWriteFailed,
                message);
        }
