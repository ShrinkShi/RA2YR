...
{
    internal static class PaletteAuditExternalManifestWriter
    {
        public static PaletteAuditExternalManifestReference Write(
            ...
            byte[] manifestBytes)
        {
            ...
            try
            {
                ...
                string relativeDirectory =
                    "wp02d/palette-audits/" + sourceId + "/" + directoryFingerprint;
                ...
                CreateVerifiedDirectory(Path.Combine(configuration.CachePath, "wp02d"));
                ...
                if (!RepositoryPathPolicy.IsInsideOrEqual(
                    ...
                    string.Equals(
                        ...
                        PathComparison))
                {
                    throw Failure("The external palette manifest escaped its cache boundary.");
                }

                if (File.Exists(targetPath))
                {
                    VerifyExistingFile(targetPath, manifestBytes, manifestSha256);
                    return new PaletteAuditExternalManifestReference(
                        relativePath,
            ...
            catch (PaletteProjectBaselineAuditException)
            {
                throw;
            }
            catch (Exception exception) when (IsExpectedFileException(exception))
            {
                throw Failure("The repository-external palette manifest write failed closed.");
            }
        }
            ...
            ExternalContentConfiguration configuration)
        {
            ...
            foreach (string protectedPath in protectedPaths)
            {
                ...
                {
                    throw Failure(
                        "The external palette cache boundary could not be verified safely.");
                }
            }
            ...
            if (RepositoryPathPolicy.TryFindUnsupportedAlias(
                ...
                RepositoryPathPolicy.ContainsExistingReparsePoint(
                    ...
                    out reparsePoint))
            {
                throw Failure("The external palette cache uses an unsupported path identity.");
            }
        }
        ...
        private static void CreateVerifiedDirectory(string path)
        {
            if (File.Exists(path))
            {
                throw Failure("An external palette cache directory is occupied by a file.");
            }

            string reparsePoint;
            if (RepositoryPathPolicy.ContainsExistingReparsePoint(path, out reparsePoint))
            {
                throw Failure("An external palette cache directory traverses a reparse point.");
            }
        ...
        private static void EnsureRegularDirectory(string path)
        {
            ...
            {
                throw Failure("An external palette cache path is not a regular directory.");
            }
        }
            ...
            string expectedSha256)
        {
            ...
            if ((attributes & FileAttributes.Directory) != 0 ||
                (attributes & FileAttributes.ReparsePoint) != 0)
            {
                throw Failure("An external palette manifest is not a regular file.");
            }
            ...
            {
                throw Failure(
                    "An existing content-addressed palette manifest does not match its expected bytes.");
            }
        }
        ...
        private static PaletteProjectBaselineAuditException Failure(string message)
        {
            return new PaletteProjectBaselineAuditException(
                PaletteProjectBaselineAuditFailureCode.ExternalManifestWriteFailed,
                message);
        }
