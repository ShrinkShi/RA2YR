...
> 项目目前处于 WP-02D 基础格式阶段，尚不可玩。仓库不包含原版游戏素材，也不提供临时单位、临时地图或替代素材。
...
WP-02A 建立目录型来源、显式优先级、来源链和仓库外 manifest；WP-02B 建立有界二进制输入、预算和诊断；WP-02C 在这些边界上增加 MIX 容器读写、加密目录、校验、嵌套挂载及 XCC 合成互操作；WP-02D 仅增加严格的 PAL 原始 RGB 解析。SHP、PCX、VXL/HVA、TMP、CSF、INI、地图 Pack、Texture2D、Shader、玩家色和剧院选择仍未实现，也不证明视觉或游戏行为兼容。受控基线命令只读访问必要字节并计算摘要；公开证据不包含文件正文、完整颜色表、绝对路径或完整文件级清单。
