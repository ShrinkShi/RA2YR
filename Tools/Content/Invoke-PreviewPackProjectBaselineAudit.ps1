...
$summary = Join-Path $runRoot 'm3-c5-preview-pack-project-baseline-summary.json'
$log = Join-Path $runRoot 'unity.log'
$arguments = @('-batchmode','-nographics','-quit','-projectPath',$project,'-executeMethod','RA2YR.Editor.PreviewPackProjectBaselineAuditCommand.Run','-ra2yrExternalContentConfig',$configuration,'-ra2yrSummaryOutput',$summary,'-logFile',$log)
$process = Start-Process -FilePath $unity -ArgumentList $arguments -PassThru -WindowStyle Hidden
try {
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) { $process.Kill(); $process.WaitForExit(); throw 'PreviewPack audit timed out.' }
    $process.Refresh()
    $exitCode = [int]$process.ExitCode
} finally { $process.Dispose() }
if ($exitCode -ne 0) { throw "Unity PreviewPack audit exited with code $exitCode." }
...
if ($null -eq $summaryObject -or $summaryObject.manifestType -ne 'RA2YR.PreviewPackProjectBaselineAuditSanitized') { throw 'The sanitized summary identity is invalid.' }
if ($summaryText -match '([A-Za-z]:\\|"payload"\s*:|"pixels"\s*:|"fileName"\s*:|"path"\s*:)' ) { throw 'The sanitized summary contains forbidden detail.' }
if ($summaryObject.status -notin @('Complete','CompleteWithFailures','CompleteWithNoCandidates')) { throw "PreviewPack audit status is '$($summaryObject.status)', not an aggregate terminal status." }
"Unity process exit code: $exitCode"
"Audit status: $($summaryObject.status)"
"Candidate entries: $($summaryObject.candidateEntryCount)"
"Exact decoded: $($summaryObject.exactDecodedCount)"
"Failed: $($summaryObject.failedCount)"
"Sanitized summary: TestResults/$runId/m3-c5-preview-pack-project-baseline-summary.json"
